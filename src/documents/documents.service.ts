...
export class DocumentsService {
  ...
  async create(createDocumentDto: CreateDocumentDto, psychologistId: string) {
    ...
    // Se não foi fornecido conteúdo, gerar automaticamente
    if (!content) {
      content = await this.generateDocumentContent(
        createDocumentDto.type,
        createDocumentDto.patientId,
        psychologistId,
        createDocumentDto.sessionIds,
        createDocumentDto.startDate,
        createDocumentDto.endDate,
      );
    }
  ...
  async findAll(psychologistId: string, pagination: PaginationDto, patientId?: string, type?: DocumentType) {
    ...
    return new PaginatedResult(decryptedDocuments, total, page, limit);
  }
  ...
  async update(id: string, updateData: Partial<CreateDocumentDto>, psychologistId: string) {
    const document = await this.prisma.document.findFirst({
      where: {
        id,
        patient: {
  ...
  async generateEvolutionReport(patientId: string, psychologistId: string, startDate?: string, endDate?: string) {
    // Buscar paciente
    const patient = await this.prisma.patient.findFirst({
      where: {
        id: patientId,
    ...
    if (sessions.length === 0) {
      // Se não há sessões completas, gerar um relatório básico
      const report = await this.buildBasicEvolutionReport(patient);
      return report;
    }

    // Gerar relatório com base nas sessões
    const report = await this.buildEvolutionReport(patient, sessions);
    return report;
  }

  async generatePsychologicalEvaluation(patientId: string, psychologistId: string, sessionIds?: string[]) {
    // Buscar paciente
    const patient = await this.prisma.patient.findFirst({
      where: {
        id: patientId,
    ...
    // Buscar sessões específicas ou primeiras sessões
    let sessions;
    if (sessionIds && sessionIds.length > 0) {
      ...
      // Pegar sessões existentes (completas ou não)
      sessions = await this.prisma.session.findMany({
        where: {
          patientId,
          psychologistId,
        },
        take: 5, // Pegar até 5 sessões
        orderBy: {
          scheduledAt: 'asc',
        },
      });
    ...
    // Gerar avaliação mesmo sem sessões completas
  ...
  /**
   * Gerar relatório de evolução enriquecido com IA do Fireflies
   */
  async generateAIEnhancedEvolutionReport(
    patientId: string, 
    ...
    // Buscar sessões com transcrições
    const whereClause: any = {
      patientId,
      psychologistId,
      status: 'COMPLETED',
    ...
    if (startDate || endDate) {
      ...
      if (endDate) whereClause.scheduledAt.lte = new Date(endDate + 'T23:59:59.999Z');
    }
    ...
    // Gerar relatório enriquecido
    return this.buildAIEnhancedEvolutionReport(patient, sessions, aiInsights);
  }

  /**
   * Gerar avaliação psicológica com insights de IA
   */
  async generateAIEnhancedPsychologicalEvaluation(
    patientId: string, 
    psychologistId: string, 
    sessionIds?: string[]
  ): Promise<string> {
  ...
  /**
   * Gerar relatório de sessão individual com IA
   */
  async generateAISessionReport(sessionId: string, psychologistId: string): Promise<string> {
    ...
    if (!session || !session.transcriptId) {
      throw new NotFoundException('Sessão com transcrição não encontrada');
    }

    // Obter insights de IA do Fireflies
    const aiInsights = await this.firefliesService.generateCustomInsights(
      session.transcriptId,
      psychologistId,
      'session_analysis'
    );

    const cfpReport = await this.firefliesService.generateAIEnhancedCFPReport(
      session.transcriptId,
      psychologistId
    );
  ...
  /**
   * Gerar relatório de progresso com análise longitudinal
   */
  async generateProgressAnalysisReport(patientId: string, psychologistId: string): Promise<string> {
    const patient = await this.prisma.patient.findFirst({
      where: { id: patientId, psychologistId }
    });
    ...
    // Obter análise de progresso da IA do Fireflies
    const progressAnalysis = await this.firefliesService.analyzePatientProgressAI(
      patientId,
      psychologistId
    );
  ...
  // ========== MÉTODOS AUXILIARES DE PROCESSAMENTO IA ==========

  private async processSessionsWithAI(sessions: any[], psychologistId: string) {
    const aiInsights = {
      overallTrends: [] as any[],
      therapeuticPatterns: [] as any[],
      emotionalEvolution: [] as any[],
      techniqueEffectiveness: [] as any[],
      patientEngagement: [] as any[],
      clinicalRecommendations: [] as Array<{category: string; recommendation: string; priority: string}>
    };

    for (const session of sessions) {
      if (session.transcriptId) {
        try {
          // Obter insights detalhados de cada sessão
          const sessionInsights = await this.firefliesService.generateCustomInsights(
            session.transcriptId,
            psychologistId,
    ...
    // Gerar recomendações clínicas consolidadas
    aiInsights.clinicalRecommendations = this.generateConsolidatedRecommendations(aiInsights);

    return aiInsights;
  }

  private async buildAIEnhancedEvolutionReport(patient: any, sessions: any[], aiInsights: any): Promise<string> {
...
`;
  ...
  private async buildAIEnhancedPsychologicalEvaluation(patient: any, sessions: any[], aiInsights: any): Promise<string> {
    const decryptedPatient = this.decryptPatientData(patient);
...
`;
  ...
  private buildAISessionReport(session: any, aiInsights: any, cfpReport: any): string {
...
`;
  ...
  private buildProgressAnalysisReport(patient: any, progressAnalysis: any): string {
    const decryptedPatient = this.decryptPatientData(patient);
...
`;
  ...
  // ========== MÉTODOS DE FORMATAÇÃO E ANÁLISE ==========

  private generateEvolutionAnalysisAI(aiInsights: any): string {
  ...
  private generateTherapeuticPatternsAnalysis(aiInsights: any): string {
  ...
  private generateConsolidatedRecommendations(aiInsights: any): Array<{category: string; recommendation: string; priority: string}> {
    ...
    if (aiInsights.patientEngagement.length > 0) {
      const avgEngagement = aiInsights.patientEngagement.reduce((acc, curr) => 
        acc + (curr.engagement?.overall_score || 0), 0) / aiInsights.patientEngagement.length;
...
Engajamento geral: ${insights.patientEngagement?.overall_score || 'N/A'}/100`;
  ...
  private formatCFPReport(cfpReport: any): string {
...
Procedimentos: ${cfpReport.technicalProcedures?.join(', ') || 'Não especificados'}`;
  ...
  private formatSessionMetrics(metrics: any): string {
    if (!metrics) return 'Métricas não disponíveis';
...
Itens de ação: ${metrics.action_items_count || 0}`;
  ...
  private formatProgressTrends(trends: any[]): string {
    if (!trends || trends.length === 0) {
      return 'Tendências de progresso serão identificadas com mais sessões analisadas.';
    }
    
    return trends.map((trend, index) => 
      `Sessão ${index + 1}: Sentimento ${trend.sentiment}, Engajamento ${trend.engagement}%, Progresso ${trend.progress}%`
    ).join('\n');
  }

  private formatEmotionalPatterns(patterns: any[]): string {
  ...
  private formatEngagementEvolution(evolution: any[]): string {
    return 'Evolução do engajamento mapeada através das sessões analisadas.';
  }

  private formatTechniqueEvolution(techniques: any[]): string {
    return 'Efetividade das técnicas avaliada ao longo das sessões.';
  }

  private formatProgressRecommendations(recommendations: any[]): string {
  ...
  // Métodos adicionais para IA
  private generateNextStepsFromAI(aiInsights: any): string {
    return 'Próximos passos baseados em análise preditiva da IA e padrões identificados nas sessões.';
  }

  private generateAIEvaluationResults(aiInsights: any): string {
  ...
  private generatePsychologicalProfileAI(aiInsights: any): string {
  ...
  private generateResourcesAndLimitationsAI(aiInsights: any): string {
  ...
  private generateTherapeuticRecommendationsAI(aiInsights: any): string {
    return 'Recomendações terapêuticas baseadas em análise preditiva de efetividade de técnicas e padrões de resposta.';
  }

  private generatePrognosisFromAI(aiInsights: any): string {
  ...
  private formatTherapeuticMoments(insights: any): string {
    return 'Momentos terapêuticos importantes identificados automaticamente durante a análise de transcrição.';
  }

  private formatSessionRecommendations(recommendations: any[]): string {
    if (!recommendations || recommendations.length === 0) {
      return 'Recomendações personalizadas serão geradas baseadas nos padrões identificados.';
    }
  ...
  private async buildBasicEvolutionReport(patient: any): Promise<string> {
    const decryptedPatient = this.decryptPatientData(patient);
...
`;
  ...
  private async buildEvolutionReport(patient: any, sessions: any[]): Promise<string> {
    const decryptedPatient = this.decryptPatientData(patient);
...
`;
  ...
  private async buildPsychologicalEvaluation(patient: any, sessions: any[]): Promise<string> {
    const decryptedPatient = this.decryptPatientData(patient);
    const decryptedSessions = sessions.map(session => this.decryptSessionData(session));
    const completedSessions = decryptedSessions.filter(s => s.status === 'COMPLETED');
...
`;
  ...
  private async generateConsentForm(patientId: string, psychologistId: string): Promise<string> {
    const patient = await this.prisma.patient.findFirst({
      where: { id: patientId, psychologistId },
    });
...
`;
  ...
  private async generateReferralTemplate(patientId: string, psychologistId: string): Promise<string> {
    const patient = await this.prisma.patient.findFirst({
      where: { id: patientId, psychologistId },
    });
...
`;
  ...
  private analyzeAdherence(sessions: any[]): string {
    const totalSessions = sessions.length;
    const completedSessions = sessions.filter(s => s.status === 'COMPLETED').length;
    
    if (totalSessions === 0) return 'A ser avaliada';
    
    const adherenceRate = (completedSessions / totalSessions) * 100;
    
    if (adherenceRate >= 80) return 'Excelente';
    if (adherenceRate >= 60) return 'Boa';
    if (adherenceRate >= 40) return 'Regular';
    return 'Necessita atenção';
  }

  private analyzeProgress(sessions: any[]): string {
    if (sessions.length === 0) {
    ...
    // Análise simples baseada na presença de palavras-chave
    ...
    sessionsWithNotes.forEach(session => {
      ...
      positiveKeywords.forEach(keyword => {
        if (notes.includes(keyword)) positiveCount++;
      });
      negativeKeywords.forEach(keyword => {
        if (notes.includes(keyword)) negativeCount++;
      });
    });
  ...
  private generateFinalConsiderations(sessions: any[]): string {
    const totalSessions = sessions.length;
...
`;
  ...
  private extractAllTechniques(sessions: any[]): string[] {
  ...
  private analyzeEmotionalState(sessions: any[]): string {
    if (sessions.length === 0) return 'A ser avaliado';
    return 'Dentro dos parâmetros esperados para o quadro apresentado';
  }

  private analyzePersonalResources(sessions: any[]): string {
    if (sessions.length === 0) return 'A serem identificados';
    return 'Demonstra capacidade de insight e engajamento no processo terapêutico';
  }

  private identifyNeeds(sessions: any[]): string {
    return 'Continuidade do acompanhamento psicológico e desenvolvimento de estratégias de enfrentamento';
  }

  private generateRecommendations(patient: any, sessions: any[]): string {
...
`;
  ...
  private decryptDocumentData(document: any) {
    ...
    sensitiveFields.forEach(field => {
      if (decrypted[field] && typeof decrypted[field] === 'string') {
        try {
          ...
          // Se não conseguir descriptografar, manter o valor original
          // Pode ser que o campo não esteja criptografado
        }
      }
    });
